...
void TIMER2_init(Timer2_configType * config_Ptr , uint8 a_initialCounter)
{
	...
	{
		SET_BIT(DDRD,PB7); // OC2 Pin as output pin
	}
	...
	/*
	 * insert the required CTC mode in bit no. 4,5 (COM20 and COM21)
	 * of TCCR2 Register
	 */
	TCCR2 = (TCCR2 & 0xCF) | ((config_Ptr->CTC_Mode) << 4);
	/*
	 * insert the required Waveform Generation mode in bit no. 3,6 (WGM20 and WGM21)
	 * of TCCR2 Register
	 */
